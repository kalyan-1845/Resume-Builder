...
// 2. Clear out workspace files (except node_modules and generator.js)
...
const commitList = [
  ...
  { msg: "fix: prevent dual page break orphan headings on template minimal", type: "tweak", file: "src/templates/TemplateMinimal.tsx" },
...
for (let i = 0; i < totalCommits; i++) {
  ...
  if (commit.type === 'add') {
    ...
    if (completedContent) {
      if (relPath.endsWith('.css') || relPath.endsWith('.html') || relPath.endsWith('.json')) {
        ...
        fs.writeFileSync(fullFilePath, completedContent, 'utf8');
      } else {
  ...
  // Git Add & Commit with backdated env
  try {
    ...
    execSync(`git commit -m "${commit.msg}"`, { env, cwd: workspace, stdio: 'ignore' });
  } catch (err) {
    console.error(`Commit ${i} failed:`, err.message);
  }
}

// 4. Restoration: Overwrite with 100% completed backed up files
console.log('--- 4. Restoring exact completed code files for final commit ---');
...
try {
  ...
  execSync('git commit -m "style: final styling polishing and full code integration pass"', { env, cwd: workspace, stdio: 'ignore' });
} catch (e) {}
